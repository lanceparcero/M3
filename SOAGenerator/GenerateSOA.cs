...
{
    public class GenerateSOA
    {
        public static string Launch(DateTime? SettlementDate, string MerchantID = "")
        {
            ...
            {
                DataTable settlementtransactions = _excel.getDailySoaHeaders(SettlementDate, MerchantID);
                DataTable transactions = _excel.getDailySoaTransactions(SettlementDate, MerchantID); //M3Maintenance.getallTransaction(getDateFrom(), MerchantTransID);
                ...
                if (SettlementDate != null)
                {
                    ...
                    if (settlementtransactions.Rows.Count != 0)
                    {
                        ...
                        ws.Cells["B10"].Value = merccon.firstname.ToString() + " " + merccon.lastname.ToString();
                        ...
                        ws.Cells["B14"].Value = merccon.email.ToString();
                        ...
                        if (transactions.Rows.Count != 0)
                        {
                            ...
                            for (int i = 0; i < transactions.Rows.Count; i++)
                            {
                                ...
                                for (int j = 0; j < 7; j++)
                                {
                                    ...
                                    ws.Cells[startrow, 9].Value = transactions.Rows[i].ItemArray[4].ToString();

                                    ws.Cells[startrow, 10].Value = transactions.Rows[i].ItemArray[5].ToString();
                                    ws.Cells[startrow, 11].Value = transactions.Rows[i].ItemArray[6].ToString();
                                    ws.Cells[startrow, 12].Value = transactions.Rows[i].ItemArray[7].ToString();
                ...
                pck.SaveAs(new FileInfo(outputfilt));
                System.Diagnostics.Process.Start(outputfilt);
            }
